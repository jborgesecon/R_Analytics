...
is_unique <- function(df, column_name) {
...
# Realizar o left_join e a filtragem
df.gc <- left_join(df.gc, df.main[bring], by = 'matricula') #%>%
        #  filter(!is.na(dt_inicio_contrato))

df.ge <- left_join(df.ge, df.main[bring], by = 'matricula') #%>%
        #  filter(!is.na(dt_inicio_contrato))

# Aplicar distinct() nos dataframes df.gc e df.ge
df.gc <- df.gc %>%
         arrange((dt_inicio_contrato)) %>%
...
print(dim(df.gc))
print(colSums(is.na(df.gc[4])))
result <- is_unique(df.gc, 'matricula')
print(result)

write.csv(df.ccusto, 'ccusto.csv')
write.csv(df.gc, 'gc.csv')
write.csv(df.ge, 'ge.csv')
